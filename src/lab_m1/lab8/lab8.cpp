...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        materialKs = 0.5;
        spotlight = 0;
        lightPosition2 = glm::vec3(-1, 1.5f, 1);
        point_light_pos[0] = lightPosition;
        point_light_pos[1] = lightPosition2;
    }
}
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1.f, 0.f, 0.f));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.5f, 0.f, 0.f));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_1)) lightPosition2 -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_2)) lightPosition2 -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_3)) lightPosition2 += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_4)) lightPosition2 += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_5)) lightPosition2 += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_6)) lightPosition2 -= up * deltaTime * speed;
