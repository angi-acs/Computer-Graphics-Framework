...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    globe = 1;
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["earth"]);
    }
    globe = 0;
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["random"], mapTextures["crate"]);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1f(locationTime, Engine::GetElapsedTime());

    int locationGlobe = glGetUniformLocation(shader->program, "globe");
    glUniform1i(locationGlobe, globe);
